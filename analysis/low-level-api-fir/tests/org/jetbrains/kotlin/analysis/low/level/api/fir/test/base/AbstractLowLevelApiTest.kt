...
abstract class AbstractLowLevelApiTest : TestWithDisposable() {
    ...
    protected fun runTest(@TestDataFile path: String) {
        ...
        val ktFiles = moduleInfo.testFilesToKtFiles.filter { (testFile, _) -> !testFile.isAdditional }.values.toList()
    ...
    protected fun <R> analyse(contextElement: KtElement, action: KtAnalysisSession.() -> R): R {
        return if (useDependedAnalysisSession) {
            ...
            org.jetbrains.kotlin.analysis.api.analyse(contextElement, action)
        }
    }
